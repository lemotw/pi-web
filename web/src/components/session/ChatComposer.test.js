...
describe('chat composer runner', () => {
  ...
  it('Ctrl+I in the textarea opens the model selector', () => {
    ...
    const event = new dom.window.KeyboardEvent('keydown', { key: 'i', ctrlKey: true, bubbles: true, cancelable: true });
    dom.window.document.getElementById('pi-chat-message').dispatchEvent(event);
  ...
  it('uses the post-compaction estimate, not stale pre-compaction usage', () => {
    ...
    runChatComposer({
      ...
      windowImpl: dom.window,
      localEntries: mockEntries,
      chatApi: { getWorkerStatus: () => Promise.resolve({ ok: true, json: () => Promise.resolve({ state: 'idle', model: 'gpt-4o', modelProvider: 'openai' }) }) },
      chatSelectors: { THINKING_LEVELS: [] },
      modelSelector: { setupModelSelector: vi.fn((opts) => { opts.setKnownModelLabel('gpt-4o @ openai'); return { open: vi.fn() }; }) },
      thinkingSelector: { setupThinkingLevelSelector: vi.fn() },
      setIntervalImpl: () => {}
    });
    dom.window.document.dispatchEvent(new dom.window.Event('DOMContentLoaded'));
  ...
  it('clicking the compact button calls /api/compact (not sendChat) and closes the popover', async () => {
    ...
    const compact = vi.fn(() => Promise.resolve(new Response('{"status":"compacting"}', { status: 200 })));
    ...
    // Feedback: the "compacting…" banner shows and the status label reads "compacting".
    expect(dom.window.document.getElementById('pi-chat-compacting-banner').hidden).toBe(false);
    expect(dom.window.document.getElementById('pi-chat-status').textContent).toBe('compacting');
  });
