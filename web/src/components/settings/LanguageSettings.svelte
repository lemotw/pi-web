<script>
  ...
  onMount(() => {
    let stored = '';
    try {
      stored = localStorage.getItem(CUSTOM_LANGUAGES_KEY) || '';
    } catch {}
    if (stored) {
      try {
        customJson = JSON.stringify(JSON.parse(stored), null, 2);
      } catch {
        customJson = stored;
      }
    }
  ...
  function saveCustomLanguages() {
    ...
    if (
      ...
      parsed.some((l) => !l || typeof l.code !== 'string' || !l.code.trim())
    ) {
...
<section class="settings-section">
  ...
  <div class="settings-row">
    <div class="settings-row-label">
      <span class="name">{t('settings.language')}</span><span class="hint"
        >{t('settings.languageHint')}</span
      >
    </div>
    <div class="settings-control">
      <select
        data-setting={localeKey}
        data-setting-locale=""
        value={locale}
        onchange={(e) => changeLocale(e.currentTarget.value)}
        >{#each locales as loc (loc.code)}<option value={loc.code}>{loc.label}</option
          >{/each}</select
      >
    </div>
  </div>
  <div class="settings-row settings-row-stacked">
    <div class="settings-row-label">
      <span class="name">{t('settings.customLanguages')}</span><span class="hint"
        >{t('settings.customLanguagesHint')}</span
      >
    </div>
    <div class="settings-control settings-control-stacked">
      <textarea
        data-custom-languages
        class="settings-custom-languages"
        rows="8"
        spellcheck="false"
        autocapitalize="off"
        autocomplete="off"
      ...
      ></textarea>
      <div class="settings-custom-languages-actions">
        <button type="button" class="btn-secondary" data-copy-en-keys onclick={copyEnglishKeys}
          >{t('settings.copyEnglishKeys')}</button
        ><button
          type="button"
          class="btn-primary"
          data-save-custom-languages
          onclick={saveCustomLanguages}>{t('settings.saveApply')}</button
        >
      </div>
      <div
        class="settings-custom-languages-status"
        class:is-error={statusError}
        data-custom-languages-status
        hidden={!status}
      >
        {status}
      </div>
